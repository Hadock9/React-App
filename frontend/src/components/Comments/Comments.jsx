...
import { Pencil, Ellipsis, X } from 'lucide-react'
...
const Comments = ({ id, urlFetch, urlPost, what_id }) => {
    ...
    const [CommentTextError, SetCommentTextError] = useState('Дане поле не може бути пустим')
    ...
    const handleDeleteComment = async (Comment_id) => {
        const response = await fetch('http://localhost:4000/api/comments/delete_comment', {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    return (
        <div className='Comments'>
            <div className='flex'>
                <h1 className='text-xl text-black font-semibold'>Коментарі {Comments.length}</h1>
            </div>
            <IsRegUser>
                Ви не зареєстровані. Коментарі можуть залишати тільки зареєстровані
                користувачі.Лайки та дизлайки можуть ставити також тільки зареєстровані користувачі
            </IsRegUser>
            <div className='My comment my-3 flex'>
                ...
                <div className='w-[100%]'>
                    <TextError TextDirty={CommentTextDirty} TextError={CommentTextError} />
                                            ...
                                            >
                                                <ul class='py-2 text-sm text-gray-600 '>
                                                    ...
                                                    <li
                                                        className='flex px-3 h-10 items-center hover:bg-gray-200	cursor-pointer'
                                                        onClick={() =>
                                                            handleDeleteComment(OneComment.id)
                                                        }
                                                    >
                                                        <X className='mr-2' />
                                                        Видалити
