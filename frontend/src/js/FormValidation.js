...
export const validateStakeAmount = (amount, balance) => {
	if (!amount) return 'Поле суми не може бути пустим'
	const numericAmount = Number(amount)
	if (!Number.isInteger(numericAmount)) {
		return `Це повинно бути ціле число `
	}

	if (numericAmount < 1 || numericAmount > balance) {
		return `Це повинно бути ціле число більше 0 та менше ніж ваш баланс${balance}`
	}
